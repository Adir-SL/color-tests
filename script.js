...
function addColor(a){
    if(a > window.countNum){
        document.getElementById("main-flex").innerHTML += '<svg id="container-1" class="color-container" width="200" height="200" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="100" cy="100" r="70" fill="#f00" stroke="#ddd"/></svg>';
        window.countNum += 1;
        addColor(6);
    }
    updateColors(a);
...
function updateColors(a){
    ...
    for (i = 0; i < x.length; i++) {
        x[i].getElementsByTagName("circle")[0].style.fill = "rgb("+ ((color_1_r / (a+(i+1-a))) + (color_2_r / (a-i))) +","+ ((color_1_g / (a+(i+1-a))) + (color_2_g / (a-i))) +","+ ((color_1_b / (a+(i+1-a))) + (color_2_b / (a-i))) +")";
    }
}
